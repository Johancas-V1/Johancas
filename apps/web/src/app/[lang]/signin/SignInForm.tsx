...
export default function SignInForm({ dict, lang }: { dict: SignInDict; lang: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : dict.loginFailed);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="w-full max-w-md bg-white rounded-2xl shadow-[0_8px_30px_rgba(0,0,0,0.08)] p-10">
      <h1 className="text-3xl font-extrabold text-[var(--navy-dark)] mb-8">{dict.signIn}</h1>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div>
          ...
          <Link href={`/${lang}/forgot-password`} className="text-xs text-[var(--blue-accent)] hover:underline mt-1.5 inline-block">
            {dict.forgotPassword}
          </Link>
        </div>
      ...
      <p className="mt-6 text-center text-sm text-[var(--text-gray)]">
        ...
        <Link href={`/${lang}/signup`} className="font-semibold text-[var(--navy-dark)] hover:underline">
          {dict.signUp}
        </Link>
      </p>
    </div>
